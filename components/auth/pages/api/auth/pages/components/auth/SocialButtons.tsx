...
type Props = {
  onContinue?: () => void; // used by guest only; not needed for Google/Apple
  layout?: "row" | "col";
};

export default function SocialButtons({ onContinue, layout = "col" }: Props) {
  const base = "flex items-center justify-center gap-2 rounded-xl w-full h-10 text-sm";

  return (
    <div className={layout === "row" ? "flex gap-3" : "flex flex-col gap-3"}>
      <Button
        variant="secondary"
      ...
      {onContinue && (
        <Button variant="secondary" className={`${base}`} onClick={onContinue}>
          Continue as Guest
        </Button>
      )}
    </div>
...
function AppleIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
