...
const MOCK_ITEMS: MenuItem[] = [
  {
    ...
    coupons: [
      ...
      { id: "c2", label: "15% off up to $4", type: "percent", value: 15, cap: 4, source: "Promo" }
    ],
    url: "#",
    distanceMi: 1.2,
  },
  {
    ...
    estFeesTax: 0.75,
    pickupAvailable: true,
    coupons: [{ id: "c3", label: "$1 off any sandwich", type: "flat", value: 1, source: "In-store" }],
    url: "#",
    distanceMi: 0.6,
  },
];
...
function applyCoupon(subtotal: number, c: Coupon) {
  ...
  if (c.type === "flat") {
    const d = Math.min(c.value, subtotal);
    return { discount: d, applied: d > 0 };
  }
...
const estimateFuelCost = (roundTripMiles: number, mpg: number, gas: number) =>
  mpg > 0 ? (roundTripMiles / mpg) * gas : 0;
...
export default function OnBudgetPrototype({ onSignOut }: { onSignOut?: () => void }) {
  ...
  const cuisines = React.useMemo(
    () => query.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean),
    [query]
  );

  const results = React.useMemo(() => {
    const matches = (i: MenuItem) =>
      cuisines.length === 0 || cuisines.some((k) => i.cuisine.join(" ").toLowerCase().includes(k));
    ...
    <div className="min-h-screen p-4 md:p-8 bg-gradient-to-b from-white to-slate-50">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-start justify-between mb-6">
          ...
          {onSignOut && (
            <Button variant="secondary" onClick={onSignOut} className="rounded-2xl">
              Sign out
            </Button>
          )}
        </div>
          ...
          <CardContent className="p-4 md:p-6 space-y-4">
            <div className="grid md:grid-cols-[1fr_220px] gap-4">
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <Input
                    ...
                    placeholder="e.g., american, burgers, sandwiches"
                  />
              ...
              <div>
                ...
                <div className="px-2">
                  <Slider value={budget} min={2} max={15} step={0.5} onChange={setBudget} />
                </div>
              </div>
            </div>
            ...
            <Card key={r.id} className={`rounded-2xl shadow-sm h-full ${r.fitsBudget ? "border-emerald-300" : ""}`}>
              <CardContent className="p-5 space-y-3 flex flex-col h-full">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-sm text-slate-600">
                      {r.vendor} • {r.cuisine.join(", ")} • {r.distanceMi?.toFixed(1)} mi
                    </div>
                  </div>
                ...
                <div className="text-sm grid grid-cols-2 gap-1">
                  <div>Base</div>
                  <div className="text-right">{formatUSD(r.basePrice)}</div>
                  <div>Fees & tax</div>
                  <div className="text-right">{formatUSD(r.estFeesTax)}</div>
                  {r.coupon && (
                    <>
                      <div className="text-slate-500">Coupon ({r.coupon.label})</div>
                      <div className="text-right text-rose-600">- {formatUSD(r.discount)}</div>
                    </>
                  )}
                  <div className="font-medium">Adjusted total</div>
                  <div className="text-right font-semibold">{formatUSD(r.adjusted)}</div>
                </div>
                <div className="mt-auto flex items-center justify-between pt-2">
                  ...
                  <div className="flex items-center gap-3">
                    <a href={r.url ?? "#"} className="text-sm underline underline-offset-4">
                      View
                    </a>
                    <Button className="rounded-xl" onClick={() => setSelectedId(r.id)}>
                      Choose
                    </Button>
                  </div>
                </div>
          ...
          <Card className="rounded-2xl shadow-sm border-emerald-300 mt-8">
            <CardContent className="p-5 md:p-6 space-y-5">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h2 className="text-xl font-semibold">
                    Best way to get your <span className="underline">{selected.name}</span>
                  </h2>
                  <div className="text-slate-600 text-sm">
                    Distance ~ {selected.distanceMi?.toFixed(1)} mi one-way
                  </div>
                </div>
                <Button variant="secondary" className="rounded-xl" onClick={() => setSelectedId(null)}>
                  Close
                </Button>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-slate-600">
                    Current: ~{walkMaxMi.toFixed(2)} mi one-way
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-[1fr_auto] gap-2 items-center">
                    <Input
                      ...
                      step="0.1"
                      value={customMPG ?? ""}
                      onChange={(e) => setCustomMPG(e.target.value ? Number(e.target.value) : undefined)}
                      placeholder="e.g., 32"
                    />
                  ...
                  <div className="grid md:grid-cols-[1fr_320px] gap-4">
                    <Card className="rounded-2xl shadow-sm">
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="space-y-2">
                          {travelAdvice.modeRanking.map((m, idx) => (
                            <div
                              key={m.mode}
                              className={`flex items-center justify-between border rounded-xl px-3 py-2 ${
                                idx === 0 ? "border-emerald-400 bg-emerald-50" : "border-slate-200"
                              }`}
                            >
                              <div>
                    ...
                    <Card className="rounded-2xl shadow-sm">
                      <CardContent className="p-4 space-y-2">
                        ...
                        <div className="text-sm grid grid-cols-2 gap-1">
                          <div>Item adjusted</div>
                          <div className="text-right">{formatUSD(selected.adjusted)}</div>
                          <div>Pickup fuel est</div>
                          <div className="text-right">{formatUSD(travelAdvice.pickupFuel)}</div>
                          <div>Delivery fees est</div>
                          <div className="text-right">{formatUSD(travelAdvice.deliveryFees)}</div>
                        </div>
                        <div className="text-sm grid grid-cols-2 gap-1 pt-2 border-t">
                          <div className="font-medium">Pickup total</div>
                          <div className="text-right font-semibold">{formatUSD(travelAdvice.pickupTotal)}</div>
                          <div className="font-medium">Delivery total</div>
                          <div className="text-right font-semibold">{formatUSD(travelAdvice.deliveryTotal)}</div>
                        </div>
                      </CardContent>
                    </Card>
                  </div>
              ...
              <div className="text-xs text-slate-500">
                Prototype with mocked data. In production: EPA MPG, local gas prices, routed distance via Maps APIs.
              </div>
            </CardContent>
          </Card>
        )}
